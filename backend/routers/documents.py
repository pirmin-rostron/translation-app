...
def list_grouped_documents(
    ...
    jobs = (
        db.query(TranslationJob)
        .filter(TranslationJob.document_id.in_(doc_ids), TranslationJob.deleted_at.is_(None))
        .order_by(TranslationJob.created_at.desc()).all()
    ) if doc_ids else []

    proj_ids = {d.project_id for d in docs if d.project_id}
    project_names = (
    ...
    if doc_ids:
        for doc_id, cnt in (
            db.query(DocumentBlock.document_id, func.sum(func.length(DocumentBlock.text_original)))
    ...
    for job in jobs:
        if job.document_id in jobs_by_doc:
            ...
            jobs_by_doc[job.document_id].append({
                "id": job.id, "target_language": job.target_language,
                "source_language": job.source_language, "status": job.status,
                "quality_score": None,
